# SMVDU Timetable Generator

### Description
The SMVDU Timetable Generator aims to create the best university timetable for any SMVDU students. Equipped with the most accurate genetic algorithm, it can create the best timetables based on user's preferences such as setting a maximum time for the latest class, or having the smallest walking distance between classes.

The SMVDU Timetable Generator project is a full stack web development project. It is comprised from several components: the Web Scrapper, SQL Database infrustructure, Data Models, Timetable Generator Library, Genetic Algorithm Analyzer, C# Web Api, and a Angular JS Front End Web Application. 
...
- Set up a new database to store the SMVDU data
- Open up the **SMVDU Database** project properties in Visual Studio, edit the connection string such that it links to your newly created database
- Select the *Create new database* setting in the Build settings of the **SMVDU Database** project properties (so that it creates the relational database structure in your new database).
- Run the **SMVDU Database** project by setting it as a *startup project* and pressing F5.

##### Set up the web scrapper and get the latest SMVDU data:
...
- Edit the connection string of the **SMVDU.dbml** file so that any updates/deletions to the database will be made to your database.
...
- Change the connection string in the **SMVDU.dbml** file to your database.
